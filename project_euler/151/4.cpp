...
map<VI, LL> doJobs(const VI& v)
{
	...
	map<VI, LL> mt = doJobs(nextPaper);
	RA(j, mt) m[j->first] += v[i] * j->second;
    }
}

LL countSingle(const map<VI, LL>& m)
{
    LL result = 0;
...
int main()
{
    ...
    map<VI, LL> m = doJobs(v);
    LL total = 0;
    RA(i, m) { total += i->second; cout<<i->second<<" : ";  print(i->first); }

    LL single = countSingle(m);
    cout << single << " : "<<total<<endl;
    cout <<"///////////"<<endl;

    cout << (double)(single)/total<<endl;
}
