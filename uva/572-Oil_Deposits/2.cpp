...
int oil(const VS& v)
{
	...
	if (v[i][j] == '@') {
	    int x = i*n+j;
	    if (i > 0 && v[i-1][j] == '@') p[x].pb(x-n);
	    if (i < m-1 && v[i+1][j] == '@') p[x].pb(x+n);
	    if (j > 0 && v[i][j-1] == '@') p[x].pb(x-1);
	    if (j < n-1 && v[i][j+1] == '@') p[x].pb(x+1);
	    if (i > 0 && j > 0 && v[i-1][j-1] == '@') p[x].pb(x-n-1);
	    if (i > 0 && j < n-1 && v[i-1][j+1] == '@') p[x].pb(x-n+1);
	    if (i < m-1 && j > 0 && v[i+1][j-1] == '@') p[x].pb(x+n-1);
	    if (i < m-1 && j < n-1 && v[i+1][j+1] == '@') p[x].pb(x+n+1);
	}
    }
