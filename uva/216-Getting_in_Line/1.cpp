...
void cable(const vector<VD>& v)
{
	...
	// print(x);
	double t = 0;
	FSZ(i, 1, x) t += sqrt((v[x[i]][0]-v[x[i-1]][0])*(v[x[i]][0]-v[x[i-1]][0])+(v[x[i]][1]-v[x[i-1]][1])*(v[x[i]][1]-v[x[i-1]][1]));
	if (t < m) { m = t; mv = x; }
	// if (x[0] > n/2) break;
    } while (next_permutation(A(x)));
    cout.setf(ios_base::fixed);
    cout << setprecision(2);
    FSZ(i, 1, mv) cout << "Cable requirement to connect ("
		       << (int)v[mv[i-1]][0] << "," << (int)v[mv[i-1]][1] << ") to (" << (int)v[mv[i]][0] << "," << (int)v[mv[i]][1]
		       << ") is " << sqrt((v[mv[i]][0]-v[mv[i-1]][0])*(v[mv[i]][0]-v[mv[i-1]][0])+(v[mv[i]][1]-v[mv[i-1]][1])*(v[mv[i]][1]-v[mv[i-1]][1]))+16 << " feet." << endl;
    cout << "Number of feet of cable required is " << m+(n-1)*16 << "." <<endl;
}
...
int main()
{
    ...
    while (cin >> n) {
	if (!n) break;
	cout << "**********************************************************" << endl << "Network #" << ++k << endl;
	vector<VD> v(n, VD(2));
	R(i, n) cin >> v[i][0] >> v[i][1];
	cable(v);
    }
}
