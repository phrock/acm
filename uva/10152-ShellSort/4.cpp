...
void climb(list<string>& lst, string s)
{
    cout << s << endl;
    list<string>::iterator a = find(A(lst), s);
    lst.pb(*a);
    lst.erase(a);
}
...
int main()
{
	...
	RA(j, l2) {
	    if (P(st, *j)) continue;
	    // list<string>::const_iterator a = find(A(l1), *j);
	    list<string> lst(l1.begin(), find(A(l1), *j));
	    for (list<string>::const_iterator x=l2.begin(); x!= j; x++) {
		// if (find(l1.begin(), a, *x) == a) {
		if (!PO(lst, *x)) {
		    FA(b, j, l2) { climb(l1, *b); st.insert(*b); }
		    break;
		}
	    }
	}
