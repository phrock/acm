...
bool check_mirrored(string s, map<char, char>& m)
{
    string t = s;
    reverse(A(t));
    RSZ(i, s) if (P(m, s[i]) && m[s[i]] == t[i]) continue; else return false;
    return true;
}
...
int main()
{
    ...
    while (cin >> s) {
	cout << s;
	bool f1 = check_palindrome(s);
	bool f2 = check_mirrored(s, m);
	if (f1) {
	    if (f2) cout << " -- is a mirrored palindrome." << endl;
	    else cout << " -- is a regular palindrome." << endl;
	} else if (f2) cout << " -- is a mirrored string." << endl;
