...
int check(string s1, string s2)
{
    ...
    set<char> t;
    int stroke = 0;
    RSZ(i, s1) s.insert(s1[i]);
    RSZ(i, s2) if (P(s, s2[i])) { t.insert(s2[i]); if (SZ(t) == SZ(s)) return 1; } else { stroke++; if (stroke == 7) return 0; }
    return -1;
}
...
int main()
{
	...
	cin >> s1 >> s2;
	int ret = check(s1, s2);
	if (ret == 1) cout << "You win." << endl;
	else if (ret == 0) cout << "You lose." << endl;
