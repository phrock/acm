...
void paint(VI& v, int& x, const string& s, int& si, int div)
{
    int k = SZ(v);
    F(i, 1, div) k /= 4;
...
int main()
{
	...
	int si1 = 0;
	int si2 = 0;
	paint(v1, x1, s1, si1, 1);
	paint(v2, x2, s2, si2, 1);
	// print(v1); print(v2);
	int ret = 0;
	R(i, m) if (v1[i] == 1 || v2[i] == 1) ret++;
	cout << "There are " << ret << " black pixels." << endl;
    }
}
