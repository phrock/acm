...
bool connect(const VVI& vv, int a, int b)
{
    VI visited(SZ(vv));
    queue<int> q;
    q.push(a);
	...
	q.pop();
	if (y == b) return true;
	if (!visited[y]) { visited[y] = 1; RSZ(i, vv[y]) q.push(vv[y][i]); }
    }

    return false;
}

bool adventure_game(const VVI& vv, const VI& energy)
{
    // print(energy);
    int n = SZ(vv);
    VI value(n, INT_MIN);
    ...
    value[0] = 100;
    R(i, n-1) RSZ(j, vv) RSZ(k, vv[j]) if (value[j] != INT_MIN && value[j]+energy[vv[j][k]] > value[vv[j][k]] && value[j]+energy[vv[j][k]] > 0) {
	value[vv[j][k]] = value[j]+energy[vv[j][k]];
    }
    if (value[n-1] > 0) return true;
    RSZ(j, vv) RSZ(k, vv[j]) if (value[j] != INT_MIN &&
				 value[vv[j][k]] != INT_MIN &&
				 value[j]+energy[vv[j][k]] > value[vv[j][k]] &&
				 connect(vv, vv[j][k], n-1))
	return true;
    return false;
}
...
int main()
{
	...
	R(i, n) {
	    ...
	    int m;
	    cin >> m;
	    R(j, m) { int x; cin >> x; x--; vv[i].pb(x); }
	}
	bool ret = adventure_game(vv, energy);
