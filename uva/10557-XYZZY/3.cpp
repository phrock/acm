...
int cyclic(const vector< set<int> >&g, int x)
{
    VI visited(SZ(g));
    queue<int> q;
    q.push(x);
...
int max_back_value(const vector< set<int> >& g, int a, int b, const VI& energy)
{
    VI value(SZ(g), INT_MIN);
    VI visited(SZ(g));
    queue<II> q;
    q.push(MP(a, energy[a]));
    while (!q.empty()) {
	II a = q.front();
	q.pop();
	if (a.X == b && a.Y > 0) return a.Y;
	if (!visited[a.X] || value[a.X] < a.Y) {
	    visited[a.X] = 1;
	    value[a.X] = a.Y;
	    RA(i, g[a.X]) q.push(MP(*i, energy[*i]+a.X));
	}
    }
    return value[b];
}
...
bool adventure_game(vector< set<int> >& g, VI& energy)
{
	...
	while (true) {
		...
		if (max_back_value(g, i, back, energy) > 0) powerfull(g, i, energy);
	    } else break;
	}
    }
