...
int main()
{
    ...
    bool zz = false;
    while (cin >> a >> b) {
	if (zz) { cout << "No carry operation." << endl; zz = false; }
	if (!a && !b) { zz = true; continue; }
	
	int ret = 0;
	int carry = 0;
	while (a || b) {
	    if (a%10 + b%10 + carry > 9) { carry = 1; ret++; }
	    else carry = 0;
	    a /= 10;
	    b /= 10;
	}
	if (!ret) cout << "No carry operation." << endl;
	else cout << ret << " carry operation"
		  << (ret==1 ? "." : "s.") << endl;
    }
}
