...
void dfs(const VS& v, int n, int r, int& m)
{
    int x = n/SZ(v);
    int y = n%SZ(v);
    if (v[x][y] != '.') return;
    bool flag = true;
    RSZ(i, v) RSZ(j, v[i]) if (v[i][j] == '@') {
	if (i == x) {
	    flag = false;
	    F(k, min(y, j)+1, max(y, j)) if (v[i][k] == 'X') { flag = true; break; }
	    if (!flag) return;
	}
	if (j == y) {
	    flag = false;
	    F(k, min(x, i)+1, max(x, i)) if (v[k][j] == 'X') { flag = true; break; }
	    if (!flag) return;
	}
    }
    if (r > m) m = r;
    VS next = v;
    next[x][y] = '@';
    F(i, n+1, SZ(v)*SZ(v[0])) dfs(next, i, r+1, m);
}
