...
void copying(list< pair<LL, VLL> >& lst, LL k)
{
    while (true) {
	// cout << SZ(lst) << endl;
	// RA(i, lst) { cout << i->X <<": "; RA(j, i->Y) cout << " " << *j; cout << endl; }
	if (SZ(lst) <= k) break;
	list< pair<LL, VLL> >::iterator it = lst.begin();
	list< pair<LL, VLL> >::iterator it2 = it;
	list< pair<LL, VLL> >::iterator p = it;
	it2++;
	LL m = LONG_MAX;
	while (it2 != lst.end()) {
	    if (it->X+it2->X <= m) {
	...
	list< pair<LL, VLL> >::iterator tmp = p;
	list< pair<LL, VLL> >::iterator tmp2 = p;
	tmp--;
	p++;
	VLL v = tmp->Y;
	RA(i, tmp2->Y) v.pb(*i);
