...
int dp(const VVI& a)
{
    VI d(SZ(a));
    RSZ(i, d) {
	R(j, i) if (a[j][0] > a[i][0] && a[j][1] > a[i][1]) checkmax(d[i], d[j]);
	d[i] += a[i][2];
    }
    // print(d);
    int res = 0;
    RSZ(i, d) checkmax(res, d[i]);
    return res;
}
...
int main()
{
	...
	R(i, n) {
	    cin >> a >> b >> c;
	    VI v(3);
	    v[2] = a; v[0] = max(b, c); v[1] = min(b, c);
	    vv.pb(v);
	    v[2] = b; v[0] = max(a, c); v[1] = min(a, c);
	    vv.pb(v);
	    v[2] = c; v[0] = max(a, b); v[1] = min(a, b);
	    vv.pb(v);
	}
	UN(vv);
	sort(A(vv), greater<VI>());
	// RSZ(i, vv) print(vv[i]);
	int res = dp(vv);
