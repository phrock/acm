...
{
    VI v;
    v.pb(0); v.pb(50); v.pb(25); v.pb(10); v.pb(5); v.pb(1);
    int m = 7490;
    VVI vv(m, VI(SZ(v)));
    RSZ(i, v) vv[0][i] = 1;
    FSZ(i, 1, vv) FSZ(j, 1, vv[i]) {
	if (i < v[j]) vv[i][j] = vv[i][j-1];
	else vv[i][j] = vv[i][j-1]+vv[i-v[j]][j];
    }

    int n;
    while (cin >> n) cout << vv[n][SZ(v)-1] << endl;
