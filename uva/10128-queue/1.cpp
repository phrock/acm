...
void dfs(const VI& v, const VI& used, int k, int p, int r, LL& ret, int x, int y, int lmax)
{
    // print(v);
    // cout <<x<<" "<<y<<endl;
    if (k == SZ(v)) { if (p==x && r==y) ret++; return; }

    RSZ(i, used) if (!used[i]) {
	VI next_v = v;
	next_v[k] = i;
	VI next_used = used;
	next_used[i] = 1;
	int next_x = x;
	if (i > lmax) { next_x++; lmax = i; }
	y = 0;
	R(j, k) if (v[j] > i) y++;
	
	dfs(next_v, next_used, k+1, p, r, ret, next_x, y, lmax);
    }
}
