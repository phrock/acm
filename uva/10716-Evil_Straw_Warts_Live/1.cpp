...
int pal(const string& s)
{
    ...
    char x = s[0];
    int k = s.find_last_of(x);
    if (k == SZ(s)-1) {
	return pal(s.substr(1, SZ(s)-2));
    } else if (!k) {
	return pal(s.substr(1))+SZ(s)/2;
    } else {
	return SZ(s)-1-k + pal(s.substr(1, k-1)+s.substr(k+1));
    }
}
...
int main()
{
	...
	RSZ(i, s) v[s[i]-'a']++;
	int odd = 0;
	RSZ(i, v) if (v[i]%2 == 1) odd++;
	if (odd > 1) cout << "Impossible" << endl;
	else cout << pal(s) << endl;
    }
}
