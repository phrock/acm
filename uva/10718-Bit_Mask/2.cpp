...
int main()
{
	...
	VI vl = mask(l);
	VI vu = mask(u);
	// print(vn); print(vl); print(vu);
	int a = -1;
	RDSZ(i, vu) if (vu[i]) { a = i; break; }
	VI v(32);
	unsigned res = 0;
	while (a >= 0) {
	    if (!vn[a]) {
	...
	// print(v);
	if (res < l) {
	    RDSZ(i, vl) if (vl[i] && !v[i]) { v[i] = 1; res += power(i); if (res >= l) break; }
	}
	if (res > u) {
	    RSZ(i, v) if (v[i] && !vu[i]) { v[i] = 0; res -= power(i); if (res <= u) break; }
	}
	cout << res << endl;
    }
}
