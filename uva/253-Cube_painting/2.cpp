...
bool check(string s1, string s2)
{
	...
	q.pop();
	if (x == s2) return true;
	y = x; r1(y); if (!P(st, y)) { st.insert(y); q.push(y); }
	y = x; R(i, 3) r1(y); if (!P(st, y)) { st.insert(y); q.push(y); }
	y = x; r2(y); if (!P(st, y)) { st.insert(y); q.push(y); }
	y = x; R(i, 3) r2(y); if (!P(st, y)) { st.insert(y); q.push(y); }
	y = x; r3(y); if (!P(st, y)) { st.insert(y); q.push(y); }
	y = x; R(i, 3) r3(y); if (!P(st, y)) { st.insert(y); q.push(y); }
    }
    return false;
}
