...
int packet(VI& v)
{
    ...
    a[2] = v[4]*5;
    ret += v[3]/4;
    if (v[3]%4) { ret++; a[3] = 4-v[3]%4; }
    while (v[2]) {
	if (a[2]) {
	    if (a[2] >= v[2]) { v[2] = 0; a[2] -= v[2]; break; }
	    else { v[2] -= a[2]; a[2] = 0; continue; }
	}
	if (a[3] == 1) {
	    a[3]--;
	    v[2]--;
	...
	if (a[3] == 2) {
	    if (v[2] <= 3) { a[1] += 18-v[2]*4; v[2] = 0; a[3] = 0; break; }
	    else { v[2] -= 3; a[3] = 0; a[1] += 6; continue; }
	}
	if (a[3] == 3) {
	    if (v[2] <= 5) { a[1] += 27-v[2]*4; v[2] = 0; a[3] = 0; break; }
	    else { v[2] -= 5; a[3] = 0; a[1] += 7; continue; }
	}
	
	ret += v[2]/9;
	if (v[2]%9) { ret++; a[1] += 36-(v[2]%9)*4; }
	v[2] = 0;
    }

    a[1] += a[3]*9+a[2]*4;
    if (a[1] >= v[1]) return ret;
    else {
	v[1] -= a[1];
	ret += v[1]/36;
	if (v[1]%36) ret++;
...
int main()
{
	...
	if (!v[1] && !v[2] && !v[3] && !v[4] && !v[5] && !v[6]) break;
	int ret = packet(v);
	cout << ret << endl;
    }
}
