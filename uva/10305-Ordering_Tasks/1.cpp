...
void dfs(const VVI& v, VI& visited, int x, VI& ret)
{
    if (!visited[x]) {
	visited[x] = 1;
	RSZ(i, v[x]) if (!visited[v[x][i]]) dfs(v, visited, v[x][i], ret);
	ret.pb(x);
    }
}

VI order_task(const VVI& v)
{
    VI visited(SZ(v));
    visited[0] = 1;
    VI ret;
...
int main()
{
	...
	VVI v(n+1);
	int x, y;
	R(i, m) { cin >> x >> y; v[y].pb(x); }
	VI ret = order_task(v);
	RSZ(i, ret) { if (i) cout << " "; cout << ret[i]; }
	cout << endl;
    }
}
