...
{
    RSZ(i, v) if (v[i] > 0) return true;
    return false;
}

int rotate(const VI& v, int x, int k, bool direction)
{
    while (k) {
	if (direction) {
	    x = (x+1)%SZ(v);
...
int main()
{
	...
	while (check(v)) {
	    ...
	    int p = rotate(v, x, a, true);
	    int q = rotate(v, y, b, false);
	    x = p;
	    y = q;
	    if (p == q) { printf("%3d", v[p]); v[p] = 0; }
	    else { printf("%3d%3d", v[p], v[q]); v[p] = 0; v[q] = 0; }
	    // print(v);
	}
	cout << endl;
    }
