...
int main()
{
    ...
    VI v; v.pb(0); v.pb(0); v.pb(INT_MAX);
    VVI g(SZ(a), v);
    RSZ(i, a) {
	int p = lower_bound(A(g), a[i], cmp2)-g.begin();
	d[i] = p+1;
	g[p] = a[i];
    }

    // RSZ(i, d) {
    // 	R(j, i) if (a[j][1] < a[i][1] && a[j][2] > a[i][2]) checkmax(d[i], d[j]);
    // 	d[i]++;
    // }
    print(d);
    int m = 0;
    RSZ(i, d) checkmax(m, d[i]);
    cout << m << endl;
    VI path;
    w = INT_MAX;
    iq = -1;
    RDSZ(i, d) {
	if (d[i] == m && a[i][1] < w && a[i][2] > iq) { path.pb(a[i][0]); m--; w = a[i][1]; iq = a[i][2]; }
    }
    reverse(A(path));
    RSZ(i, path) cout << path[i] << endl;
}
