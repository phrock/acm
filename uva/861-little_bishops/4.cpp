...
void helper(int n, int k, const set<int> s, int got, int start, LL& ret)
{
    if (got == k) { ret++; return; }
	...
	if (i == 0) {
		...
		set<int> next_s = s;
		next_s.insert(2*j);
		helper(n, k, next_s, got+1, i+1, ret);
	    }
	} else {
	    int x = (i-1)/2;
	    R(j, n-x-1) if (!P(s, x+1+2*j)) {
		set<int> next_s = s;
		next_s.insert(x+1+2*j);
		helper(n, k, next_s, got+1, i+1, ret);
	    }
	}
    }
