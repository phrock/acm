...
void helper(const VVI& vv, int n, int got, int k, const set<int>& diff, const set<int>& sum, LL& ret, int start)
{
    // cout << start<<endl;
    // RSZ(i, vv) print(vv[i]);
    // cout <<endl;
	...
	int a = i/n;
	int b = i%n;
	if (!P(diff, b-a) && !P(sum, a+b)) {
	    set<int> next_diff = diff;
	    next_diff.insert(b-a);
	    set<int> next_sum = sum;
	    next_sum.insert(a+b);
	    VVI next_vv = vv;
	    next_vv[a][b] = 1;
	    helper(next_vv, n, got+1, k, next_diff, next_sum, ret, i+1);
	}
    }
}

LL bishop(int a, int b)
{
    if (b > a) return 0;
    LL ret = 0;
    set<int> diff, sum;
    VVI vv(a, VI(a));
    helper(vv, a, 0, b, diff, sum, ret, 0);
    return ret;
}
