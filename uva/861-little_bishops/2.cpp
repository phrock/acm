...
void helper(int n, int got, int k, const set<int>& diff, const set<int>& sum, LL& ret, int start)
{
	...
	if (!P(diff, b-a) && !P(sum, a+b)) {
	    ...
	    helper(n, got+1, k, next_diff, next_sum, ret, i+1);
	}
    }
}
