...
void helper(VVI& vv, int k, int got, int start, LL& ret)
{
    if (got == k) { ret++; return; }

    FSZ(i, start, vv) RSZ(j, vv[i]) if (vv[i][j] == 0) {
	bool flag = true;
	R(x, i) if (vv[x][j] == 1) { flag = false; break; }
	if (flag) {
	    VVI next_vv = vv;
	    next_vv[i][j] = 1;
	    helper(next_vv, k, got+1, i+1, ret);
	}
    }
}
...
LL bishop(int n, int k)
{
    if (k > n) return 0;
    LL ret = 0;
    VVI vv(2*n-1, VI(2*n-1, -1));
    R(i, n) vv[0][2*i] = 0;
    R(i, n-1) R(j, 2) R(x, n-i-1) vv[1+2*i+j][i+1+2*x] = 0;
