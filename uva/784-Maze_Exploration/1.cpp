...
void dfs(const VVI& v, int p, VI& discovered, VS& vs)
{
    if (!discovered[p]) {
	discovered[p] = 1;
	vs[p/SZ(vs[0])][p%SZ(vs[0])] = '#';
	RSZ(i, v[p]) dfs(v, v[p][i], discovered, vs);
    }
}
...
void room(VS& vs)
{
	...
	if (vs[i][j] == ' ' || vs[i][j] == '*') {
	    int x = i*m+j;
	    if (vs[i][j] == '*') { p = x; }
	    if (i > 0 && (vs[i-1][j] == ' ' || vs[i-1][j] == '*')) v[x].pb(x-m);
	    if (i < n-1 && (vs[i+1][j] == ' ' || vs[i+1][j] == '*')) v[x].pb(x+m);
	    if (j > 0 && (vs[i][j-1] == ' ' || vs[i][j-1] == '*')) v[x].pb(x-1);
	    if (j < m-1 && (vs[i][j+1] == ' ' || vs[i][j+1] == '*')) v[x].pb(x+1);
	}
    }

    VI discovered(n*m);
    dfs(v, p, discovered, vs);
}
