...
void poetry(string a, string b)
{
    ...
    int y2 = a.find(">", x2);
    s1 = a.substr(0, x1);
    s2 = a.substr(x1+1, y1-x1-1);
    s3 = a.substr(y1+1, x2-y1-1);
    s4 = a.substr(x2+1, y2-x2-1);
    s5 = a.substr(y2+1);

    string t = b.substr(0, SZ(b)-3);
    cout << s1+s2+s3+s4+s5 << endl;
    cout << t+s4+s3+s2+s5 << endl;
}
