...
int main()
{
	...
	R(i, m) cin >> b[i];
	VI v;
	RSZ(i, a) FSZ(j, i+1, a) v.pb(a[i]+a[j]);
	UN(v);
	RSZ(i, b) {
	    cout << "Closest sum to " << b[i] << " is ";
	    int p = lower_bound(A(v), b[i])-v.begin();
	    if (p == SZ(v)) cout << v[p-1] << "." << endl;
	    else if (!p) cout << v[p] << "." << endl;
	    else if (v[p]-b[i] < b[i]-v[p-1]) cout << v[p] << "." << endl;
