...
int main()
{
	...
	VS vs(n);
	int l = 0;
	RSZ(i, vs) { getline(cin, vs[i]); checkmax(l, SZ(vs[i])); }
	int c = (column+2)/(l+2);
	int r = n/c;
	int t = n%c;
	if (t) r++; else t = c;
	cout << l << " " << c << " "<< r << endl;
	sort(A(vs));
	R(i, r) R(j, c) { int x; if (j <= t) x = j*r+i; else x = t*r+(j-t)*(r-1)+i; if (x < n) { cout << vs[x]; if (j < c-1) R(k, l-SZ(vs[x])+2) cout << " "; else { R(i, l-SZ(vs[x])) cout << " "; cout << endl; } } }
	if (r*c > n) cout << endl;
    }
}
