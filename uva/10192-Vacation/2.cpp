...
int roll_lcs(const string& s1, const string& s2)
{
    VVI d(2, VI(SZ(s2)));
    FSZ(i, 1, s1) FSZ(j, 1, s2) {
	if (s1[i] == s2[j]) d[i%2][j] = d[1-i%2][j-1]+1;
	else d[i%2][j] = max(d[i%2][j-1], d[1-i%2][j]);
    }
    return d[1-SZ(s1)%2][SZ(s2)-1];
}
