...
int main()
{
	...
	VI code(n);
	VI guess(n);
	RSZ(i, code) cin >> code[i];
	while (true) {
	    RSZ(i, guess) cin >> guess[i];
	    if (!guess[0]) break;
	    int x = 0;
	    int y = 0;
	    VI v1(n);
	    VI v2(n);
	    R(i, n) if (code[i] == guess[i]) { x++; v1[i] = 1; v2[i] = 1; }
	    RSZ(i, code)
		if (v1[i]) continue;
		else RSZ(j, guess) if (v1[i] || v2[j]) continue;  else if (code[i] == guess[j] && i != j) { y++; v1[i] = 1; v2[i] = 1; }
	    cout << "(" << x << "," << y << ")" << endl;
	}
    }
}
