...
bool is_solvable(const VVI& orig)
{
    int x = 0;
    RSZ(i, orig) RSZ(j, orig[i]) {
	R(p, i) RSZ(q, orig[i]) if (orig[p][q] != 0 && orig[p][q] < orig[i][j]) x++;
	R(q, j) if (orig[i][q] != 0 && orig[i][q] < orig[i][j]) x++;
    }

    int i = 0;
    int j = 0;
    int y = 1;
    R(k, SZ(orig)*SZ(orig[0])) if (orig[k/4][k%4] == 0) { i = k/4; j = k%4; break; }

    if ((i+j)%2) y = 0;
    // cout <<x<<" "<<x%2<<" "<<i+j<<" "<<x+y<<endl;
    if ((x+y)%2) return false; else return true;
}

void fifteen_puzzle(const VVI& orig, const VVI& dst)
{
    if (!is_solvable(orig)) { cout << "This puzzle is not solvable." << endl; return; }
	...
	if (j > 0) {
	    ...
	    if (!P(s, next.X)) { s.insert(next.X); next.Y += "L"; q.push(next); }
	}
	if (j < 3) {
	    next = x;
	    swap(next.X[i][j], next.X[i][j+1]);
	    if (!P(s, next.X)) { s.insert(next.X); next.Y += "R"; q.push(next); }
	}
	if (i > 0) {
	    next = x;
	    swap(next.X[i][j], next.X[i-1][j]);
	    if (!P(s, next.X)) { s.insert(next.X); next.Y += "U"; q.push(next); }
	}
	if (i < 3) {
	    next = x;
	    swap(next.X[i][j], next.X[i+1][j]);
	    if (!P(s, next.X)) { s.insert(next.X); next.Y += "D"; q.push(next); }
	}
    }
}
