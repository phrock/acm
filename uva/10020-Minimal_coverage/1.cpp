...
void coverage(VII& v, int m)
{
	...
	flag = false;
	FSZ(i, k, v) {
	    if (v[i].X > x) { k = i; break; }
	    if (v[i].Y > tmp.Y) { tmp = v[i]; flag = true; }
	}
	if (flag) { x = tmp.Y; ret.pb(tmp); }
	if (x >= m) break;
    }
    // RSZ(i, v) cout << v[i].X << " " <<v[i].Y << endl;
    sort(A(ret));
    if (x >= m) { cout << SZ(ret) << endl; RSZ(i, ret) cout << ret[i].X << " " <<ret[i].Y << endl; }
    else cout << 0 << endl;
}
