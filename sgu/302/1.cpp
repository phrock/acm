...
int main()
{
    ...
    R(i, n) {
	if (s[i] == '<') {
	    if (i+SZ(up)-1 < n && s.substr(i, SZ(up)) == up)
		st.push(1), i += SZ(up)-1;
	    if (i+SZ(un_up)-1 < n && s.substr(i, SZ(un_up)) == un_up)
		st.pop(), i += SZ(un_up)-1;
	    if (i+SZ(down)-1 < n && s.substr(i, SZ(down)) == down)
		st.push(2), i += SZ(down)-1;
	    if (i+SZ(un_down)-1 < n && s.substr(i, SZ(un_down)) == un_down)
		st.pop(), i += SZ(un_down)-1;
	} else if (st.empty()) cout << s[i];
	else if (st.top() == 1) cout << (char)toupper(s[i]);
