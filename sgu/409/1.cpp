...
void dfs(VS& v, int m, bool& flag, int n, int k)
{
    if (flag) return;
    if (m == k*n*n) { RSZ(i, v) cout << v[i]<<endl; flag = true; return; }
    R(i, n*n) R(j, n*n) if (v[i][j] == '.') {
	if (flag) return;
	VS next = v;
	next[i][j] = '*';
	int x=0;
	R(t, n*n) if (next[i][t] == '*') x++;
	if (x > k) continue;
	x = 0;
	R(t, n*n) if (next[t][j] == '*') x++;
	if (x > k) continue;
	x = 0;
	F(t, i/n, i/n+n) F(t2, j/n, j/n+n) if (next[t][t2] == '*') x++;
	if (x > k) continue;
	RSZ(i, next) cout << next[i]<<endl; cout << endl;
	dfs(next, m+1, flag, n, k);
    }
}
