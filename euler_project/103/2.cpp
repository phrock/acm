...
void findSet(int a, int b, const set< set<int> >& ss, int n)
{
    //    cout<<"********";   print(s);
    RA(i, ss) {
	if ((*i).size()==n) { if (isSpecialSet(*i)) print(*i); }
    }
    set< set<int> > r;
    RA(i, ss) {
	FE(j, a, b) if (find(ALL(*i), j) == (*i).end()) {
	    set<int> t = *i;
	    t.insert(j);
	    if (isSpecialSet(t) && t.size()<=n) r.insert(t);
	}
    }
    if (r.size() != 0) findSet(a, b, r, n);
}
