...
{
    VVLL vv(digits, VLL(10));

    RSZ(i, vv[0]) vv[0][i] = 1;
    FSZ(i, 1, vv) RSZ(j, vv[i]) FE(k, j, 9) vv[i][j] += vv[i-1][k];

    LL r = 0;
    RSZ(i, vv[digits-1]) r += vv[digits-1][i];

    return r-1;
}

LL decreaseAmount(int digits)
{
    LL r = 0;
    FE(i, 1, digits) r += increaseAmount(i);
    return r;
}    
...
int main()
{
    ...
    cout << increaseAmount(10)<<endl;
    cout << decreaseAmount(10)<<endl;
    cout << nonBouncyAmount(10)<<endl;
    cout <<"**********"<<endl;
    cout << nonBouncyAmount(100)<<endl;
}
