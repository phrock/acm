...
void dfs(const VVI& vv, VI& explored, int start, int& m)
{
    if (explored[start] == 1) return;
    m++;
    explored[start] = 1;

    RSZ(i, vv[start]) if (vv[start][i] != -1) dfs(vv, explored, i, m);
}
...
VVI buildMinPath(const VVI& vv)
{
	...
	int p = INT_MAX;
	R(i, n) F(j, i+1, n) {
	    if (vv[i][j]!=-1 && path[i][j]==-1 && vv[i][j]<p) {
		p = vv[i][j];
		x = i;
		y = j;
	    }
