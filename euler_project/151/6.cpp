...
map<VI, long double> doJobs(const VI& v, long double x,
			    map< VI, map<VI, long double> >& memory)
{
	...
	map<VI, long double> mt = memory[nextPaper];
	if (mt.size() == 0) {
	    mt = doJobs(nextPaper, 1, memory);
	    memory[nextPaper]=mt;
	}
	RA(j, mt) m[j->first] += v[i] * j->second * x / papers;
    }
...
int main()
{
    ...
    map< VI, map<VI, long double> >memory;
    map<VI, long double> m = doJobs(v, 1, memory);
    long double total = 0;
