...
VVLL checkout(const VVLL& scores, int n, VVI& q)
{
    int s = scores.size();
	...
	if (i < product) { vv[i][j] = vv[i][j-1]; q[i][j] = q[i][j-1]; }
	else { vv[i][j] = vv[i-product][j] + vv[i][j-1];
	    if (q[i-product][j] == -1) q[i][j] = -1;
	    else if (q[i-product][j] == 3) q[i][j] = -1;
	    else q[i][j] = q[i][j-1]+1;
	}
...
int main()
{
    ...
    VVLL scores = createScores();
    VVLL vv = checkout(scores, 100);
    
    //    set< vector<VLL> > ss = checkResult(scores, vv, 100, scores.size());    
