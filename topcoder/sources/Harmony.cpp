#include <cctype>
#include <climits>
#include <cmath>
#include <cstdio>
#include <cstdlib>
#include <cstring>
#include <algorithm>
#include <bitset>
#include <deque>
#include <fstream>
#include <iomanip>
#include <iostream>
#include <list>
#include <map>
#include <numeric>
#include <queue>
#include <set>
#include <sstream>
#include <stack>
#include <string>
#include <utility>
#include <vector>
using namespace std;

class Harmony
{ 
public: 
  vector <int> mostHarmonious(vector <int> frequencies);
  
// BEGIN CUT HERE
	public:
	void run_test(int Case) { if ((Case == -1) || (Case == 0)) test_case_0(); if ((Case == -1) || (Case == 1)) test_case_1(); if ((Case == -1) || (Case == 2)) test_case_2(); if ((Case == -1) || (Case == 3)) test_case_3(); }
	private:
	template <typename T> string print_array(const vector<T> &V) { ostringstream os; os << "{ "; for (typename vector<T>::const_iterator iter = V.begin(); iter != V.end(); ++iter) os << '\"' << *iter << "\","; os << " }"; return os.str(); }
	void verify_case(int Case, const vector <int> &Expected, const vector <int> &Received) { cerr << "Test Case #" << Case << "..."; if (Expected == Received) cerr << "PASSED" << endl; else { cerr << "FAILED" << endl; cerr << "\tExpected: " << print_array(Expected) << endl; cerr << "\tReceived: " << print_array(Received) << endl; } }
	void test_case_0() { int Arr0[] = {200,250,400,320,350}; vector <int> Arg0(Arr0, Arr0 + (sizeof(Arr0) / sizeof(Arr0[0]))); int Arr1[] = { 200,  250,  400 }; vector <int> Arg1(Arr1, Arr1 + (sizeof(Arr1) / sizeof(Arr1[0]))); verify_case(0, Arg1, mostHarmonious(Arg0)); }
	void test_case_1() { int Arr0[] = {440, 320, 750,660, 500,550}; vector <int> Arg0(Arr0, Arr0 + (sizeof(Arr0) / sizeof(Arr0[0]))); int Arr1[] = { 440,  550,  660 }; vector <int> Arg1(Arr1, Arr1 + (sizeof(Arr1) / sizeof(Arr1[0]))); verify_case(1, Arg1, mostHarmonious(Arg0)); }
	void test_case_2() { int Arr0[] = {1960,1000,3050,2341,7253,7864,2000,2352,2940,1534,7234}; vector <int> Arg0(Arr0, Arr0 + (sizeof(Arr0) / sizeof(Arr0[0]))); int Arr1[] = { 1960,  2352,  2940 }; vector <int> Arg1(Arr1, Arr1 + (sizeof(Arr1) / sizeof(Arr1[0]))); verify_case(2, Arg1, mostHarmonious(Arg0)); }
	void test_case_3() { int Arr0[] = {100,200,300,400,500,600,700,800,900,1000}; vector <int> Arg0(Arr0, Arr0 + (sizeof(Arr0) / sizeof(Arr0[0]))); int Arr1[] = { 100,  200,  400 }; vector <int> Arg1(Arr1, Arr1 + (sizeof(Arr1) / sizeof(Arr1[0]))); verify_case(3, Arg1, mostHarmonious(Arg0)); }

// END CUT HERE
 
}; 
// BEGIN CUT HERE 
////////////////////////////////////////////////////////////////////////////////
template<class T> inline void print(T A) { cout<<"{ "; for (__typeof((A).begin()) i=(A).begin(); i!=(A).end(); ++i) cout<<*i<<", "; cout<<"}"<<endl; }
template<class T> inline void print2(T A) { cout<<"{"<<endl; for (__typeof((A).begin()) i=(A).begin(); i!=(A).end(); ++i) {cout<<" { "; for (__typeof((*i).begin()) j=(*i).begin(); j!=(*i).end(); j++) cout<<*j<<", "; cout<<"}"; cout<<endl;} cout<<"}"<<endl; }
template<class T> inline void printp(T A) { cout<<"{ "; for (__typeof((A).begin()) i=(A).begin(); i!=(A).end(); ++i) {cout<<"{"<<i->first<<", "<<i->second<<"}"; cout<<", ";} cout<<" }"<<endl; }
template<class T> inline void print(T A[], int n) { cout<<"{ "; for (int i=0; i<n; i++) { cout<<A[i]; if (i+1<n) cout<<", ";} cout<<" }"<<endl; }
template<class T> inline void print2(T A[], int n, int m) { cout<<"{"<<endl; for (int i=0; i<n; ++i) {cout<<" { "; for (int j=0; j<m; ++j) { cout<<A[i][j]; if (j+1<m) cout<<", ";} cout<<" } "<<endl; } cout<<"}"<<endl; }

int main()
{
  Harmony ___test; 
  ___test.run_test(-1); 
} 
////////////////////////////////////////////////////////////////////////////////
// END CUT HERE 

bool check(const vector<int> &a, const vector<int> &b)
{
  vector<int> v1, v2;
  for (int i=0; i<(int)(a).size(); ++i)
    for (int j=i+1; j<(int)(a).size(); ++j)
      v1.push_back(a[j]/__gcd(a[i], a[j]));
  sort((v1).begin(), (v1).end(), greater<int>());
  for (int i=(int)(a).size()-1; i>=0; --i) v1.push_back(a[i]);
  for (int i=0; i<(int)(b).size(); ++i)
    for (int j=i+1; j<(int)(b).size(); ++j)
      v2.push_back(b[j]/__gcd(b[i], b[j]));
  sort((v2).begin(), (v2).end(), greater<int>());
  for (int i=(int)(b).size()-1; i>=0; --i) v2.push_back(b[i]);
  #ifdef DEBUG
  print(a); print(b);
  cout<<"???"<<endl;
  print(v1); print(v2);
  cout<<"/////////////"<<endl;
  #endif
  return v1>v2;
}

vector <int> Harmony :: mostHarmonious(vector <int> frequencies)
{
  sort((frequencies).begin(), (frequencies).end(), greater<int>());
  vector<int> res;
  int a=frequencies[0], b=frequencies[1], c=frequencies[2];
  res.push_back(a); res.push_back(b); res.push_back(c);
  for (int i=0; i<(int)(frequencies).size(); ++i)
    for (int j=i+1; j<(int)(frequencies).size(); ++j)
      for (int k=j+1; k<(int)(frequencies).size(); ++k) {
	vector<int> t;
	t.push_back(frequencies[i]); t.push_back(frequencies[j]); t.push_back(frequencies[k]);
	if (check(res, t)) res=t;
      }
  sort((res).begin(), (res).end());
  return res;
}
