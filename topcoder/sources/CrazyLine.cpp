...
int CrazyLine :: maxCrazyness(vector <int> heights)
{
  ...
  if (n==1) return 0;
  for (int i=0; i<n/2; ++i) res+=2*heights[n-i-1], res-=2*heights[i];
  if (n%2==0) res+=heights[n/2-1], res-=heights[n/2];
  else res+=max(heights[n/2]-heights[n/2+1], -heights[n/2]+heights[n/2-1]);
  return res;
}
