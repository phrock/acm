...
int GolfScore :: tally(vector <int> parValues, vector <string> scoreSheet)
{
  ...
  for (int i=0; i<(int)(parValues).size(); ++i) {
    string s=scoreSheet[i];
    if (s=="triple bogey") res+=parValues[i]+3;
    if (s=="double bogey") res+=parValues[i]+2;
    if (s=="bogey") res+=parValues[i]+1;
    if (s=="par") res+=parValues[i];
    if (s=="birdie") res+=parValues[i]-1;
    if (s=="eagle") res+=parValues[i]-2;
    if (s=="albatross") res+=parValues[i]-3;
    if (s=="hole in one") res+=1;
  }
  return res;
}
