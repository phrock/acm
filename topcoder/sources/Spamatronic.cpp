...
vector <int> Spamatronic :: filter(vector <string> knownSpam, vector <string> newMail)
{
  ...
  for (int i=0; i<(int)(knownSpam).size(); ++i) {
    string s=format(knownSpam[i]);
    vector<string> aux=split(s);
    st.insert((aux).begin(), (aux).end());
  }
  #ifdef DEBUG
  print(st);  
  #endif
  ...
  for (int i=0; i<(int)(newMail).size(); ++i) {
    string s=format(newMail[i]);
    vector<string> aux=split(s);
    sort((aux).begin(), (aux).end()), (aux).erase(unique((aux).begin(), (aux).end()), (aux).end());
    int cnt=0;
    for (int j=0; j<(int)(aux).size(); ++j)
      if (((st).find(aux[j])!=(st).end())) cnt++;
    if (cnt*4>=(int)(aux).size()*3) st.insert((aux).begin(), (aux).end());
