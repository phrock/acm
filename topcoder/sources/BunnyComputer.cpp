...
int func(const vector<int> &v)
{
  ...
  dp[1]=v[0]+v[1];
  for (int i=2; i<(int)(v).size(); ++i) dp[i]=max(dp[i-1], v[i]+v[i-1]+dp[i-2]);
  return dp.back();
}
...
int BunnyComputer :: getMaximum(vector <int> preference, int k)
{
  ...
  for (int i=0; i<(int)(preference).size(); ++i) v[i%(k+1)].push_back(preference[i]);
  int res=0;
  for (int i=0; i<(int)(v).size(); ++i) res+=func(v[i]);
  return res;
}
