...
// BEGIN CUT HERE
	...
	void test_case_3() { string Arr0[] = {"XW?", "LX?", "??X"}; vector <string> Arg0(Arr0, Arr0 + (sizeof(Arr0) / sizeof(Arr0[0]))); int Arg1 = 2; verify_case(3, Arg1, find(Arg0)); }
    	void test_case_4() { string Arr0[] = {"XW?L", "?X??", "W?XL", "??WX"}; vector <string> Arg0(Arr0, Arr0 + (sizeof(Arr0) / sizeof(Arr0[0]))); int Arg1 = 3; verify_case(4, Arg1, find(Arg0)); }
...
int TheBasketballDivTwo :: find(vector <string> table)
{
    ...
    for (int i = 0; i < (1 << m); ++i) {
        ...
        for (int j = 0; j < m; ++j) {
            int k = j + j / n + 1;
            int x = k / n, y = k % n;
            if ((i & (1 << j)) && table[x][y] == 'L') {
                ok = false;
                break;
            }
            if (!(i & (1 << j)) && table[x][y] == 'W') {
                ok = false;
                break;
            }
            if (i & (1 << j))
        ...
        if (ok) {
            res = min(res, *max_element((v).begin(), (v).end()));
        }
    }
    return res;
}
