...
int main()
{
    ...
    while (cin >> a >> b >> c >> d) {
	if (a == -1) break;
	if (d < a) x = ma-(a-d)%ma; else x = (d-a)%ma;
	if (d < b) y = mb-(b-d)%mb; else y = (d-b)%mb;
	if (d < c) z = mc-(c-d)%mc; else z = (d-c)%mc;
	int res = 1;
	while (true) {
	    x = (x+1)%ma;
	    y = (y+1)%mb;
	...
	cout << "Case " << ++k << ": the next triple peak occurs in " << res << " days." << endl;
    }
}
