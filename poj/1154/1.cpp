...
void dfs(const VS& g, int x, int y, const VI& v, int k, int& res)
{
    if (k > res) res = k;
    if (x > 0 && !v[g[x-1][y]-'A']) { VI v2 = v; v2[g[x-1][y]-'A'] = 1; dfs(g, x-1, y, v2, k+1, res); }
    if (x < SZ(g)-1 && !v[g[x+1][y]-'A']) { VI v2 = v; v2[g[x+1][y]-'A'] = 1; dfs(g, x+1, y, v2, k+1, res); }
    if (y > 0 && !v[g[x][y-1]-'A']) { VI v2 = v; v2[g[x][y-1]-'A'] = 1; dfs(g, x, y-1, v2, k+1, res); }
    if (y < SZ(g[0])-1 && !v[g[x][y+1]-'A']) { VI v2 = v; v2[g[x][y+1]-'A'] = 1; dfs(g, x, y+1, v2, k+1, res); }
}
